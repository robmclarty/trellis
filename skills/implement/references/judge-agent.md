...
<paste the current checklist from {specsDir}/.state/implement-state.md>
...
`{specsDir}/.state/implement-state.md` and report it rather than guessing.
...
2. For each criterion marked FAIL or UNCLEAR, add the judge's explanation to
   the iteration log in `{specsDir}/.state/implement-state.md`
