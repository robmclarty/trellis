...
2. **Ralph mode** — Auto-detected from the `with ralph` invocation modifier.
   ...
   `Ralph mode: on` in `{specsDir}/{feature}/implement-state.md`. Otherwise, set it to `off`.
...
Store all configuration answers in `{specsDir}/{feature}/implement-state.md` under a `## Config`
