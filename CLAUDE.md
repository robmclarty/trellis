...
- **`{specsDir}/.state/implement-state.md`**: Filesystem-based memory for the implement skill. Survives context resets and enables Ralph mode (context-fresh iterations).
- **Ralph security model**: Ralph iterations run without `--dangerously-skip-permissions`. The loop script generates `.claude/settings.local.json` with scoped permissions derived from the oracle pipeline config (only the user's toolchain commands, file tools, and git reads). Pre-flight scripts run in the loop script (outside Claude's context) and write `{specsDir}/.state/implement-preflight.json` so Claude needs no python3 access.
