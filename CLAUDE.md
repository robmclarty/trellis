...
- **`{specsDir}/{feature}/implement-state.md`**: Filesystem-based memory for the implement skill, stored alongside the feature's other artifacts. Survives context resets and enables Ralph mode (context-fresh iterations). Each feature has its own state file, enabling parallel implementations.
- **Ralph security model**: Ralph iterations run without `--dangerously-skip-permissions`. The loop script generates `.claude/settings.local.json` with scoped permissions derived from the oracle pipeline config (only the user's toolchain commands, file tools, and git reads). Pre-flight scripts run in the loop script (outside Claude's context) and write `{specsDir}/{feature}/implement-preflight.json` so Claude needs no python3 access. Ralph and Ralphd support `--stream` and `--tail` output modes for real-time observability.
