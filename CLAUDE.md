...
- **Ralph security model**: Ralph iterations run without `--dangerously-skip-permissions`. The loop script generates `.claude/settings.local.json` with scoped permissions derived from the oracle pipeline config (only the user's toolchain commands, file tools, and git reads). Pre-flight scripts run in the loop script (outside Claude's context) and write `.implement-preflight.json` so Claude needs no python3 access.
